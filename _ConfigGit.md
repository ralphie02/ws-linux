...
git config user.email || git config --global user.email $GIT_CONF_EMAIL && echo -e "Setting git user.email to $GIT_CONF_EMAIL"
git config user.name || git config --global user.name $GIT_CONF_FNAME$GIT_CONF_LNAME && echo -e "Setting git user.name to $GIT_CONF_FNAME$GIT_CONF_LNAME"
